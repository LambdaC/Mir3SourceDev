...
VOID CInventoryWnd::CreateInventoryWnd(INT nID, CWHWilImageData* pxWndImage, INT nFrameImgIdx, INT nStartX, INT nStartY, INT nWidth, INT nHeight, BOOL bCanMove)
{
	...
	// 버튼생성.m_rcWnd.left+236, m_rcWnd.top+420
...
VOID CInventoryWnd::SetInventItemNum(INT nMakeIndex, BYTE bNum)
{
	for( INT nCnt = 0; nCnt < _MAX_INVEN_ITEM; nCnt++ )
	{
		if( m_stInventoryItem[nCnt].xItem.m_stItemInfo.nMakeIndex == nMakeIndex )
			m_stInventoryItem[nCnt].xItem.m_stItemInfo.bDrugNum = bNum;
	}

	for( INT nCnt = 0; nCnt < _MAX_BELT_ITEM; nCnt++ )
	{
		if( m_stBeltItem[nCnt].xItem.m_stItemInfo.nMakeIndex == nMakeIndex )
			m_stBeltItem[nCnt].xItem.m_stItemInfo.bDrugNum = bNum;
	}
}
...
//	}
	...
	g_xMainWnd.PutsHan(NULL, rcTitle, RGB(200, 200, 255), RGB(0, 0, 0), "[관범]", g_xMainWnd.CreateGameFont("芥竟", 12, 0, FW_BOLD));
	...
	g_xMainWnd.PutsHan(NULL, rcMoney, RGB(200, 200, 255), RGB(0, 0, 0), szMoney, g_xMainWnd.CreateGameFont("芥竟", 10, 0, FW_BOLD));
					 ...
					 
				{
					if ( !m_stBeltItem[pstCommonItemSet->xItem.m_shCellNum].bSetted )
					{
						...
						if( m_stInventoryItem[nCnt].xItem.m_stItemInfo.bDrugNum == 0 )
							DeleteInvenItem(nCnt);//侶몸뒈렘壇맡맣寧맣
					}
					break;
				}
			}
...
BOOL CInventoryWnd::OnKeyUp(LPCOMMONITEMSET pstCommonItemSet, WPARAM wParam, LPARAM lParam)
{
	...
	if ( nBeltInventoryNum != -1 )
	{
		...
		{			
			...
			g_xGameProc.m_pSocket->SendItemIndex(CM_EAT, pstCommonItemSet->xItem.m_stItemInfo.nMakeIndex, pstCommonItemSet->xItem.m_stItemInfo.stStdItem.szName);
			return TRUE;
		}
	}
...
BOOL CInventoryWnd::OnLButtonUp(LPCOMMONITEMSET pstCommonItemSet, POINT ptMouse)
{
	...
	if ( pstCommonItemSet->bSetted && pstCommonItemSet->bIsEquipItem && !pstCommonItemSet->bWaitResponse )
	{
		...
		if ( nInventoryNum != -1 )
		{
			g_xGameProc.m_pSocket->SendTakeOnOffItem(CM_TAKEOFFITEM, pstCommonItemSet->xItem.m_shCellNum, pstCommonItemSet->xItem.m_stItemInfo.stStdItem.szName, pstCommonItemSet->xItem.m_stItemInfo.nMakeIndex);
			pstCommonItemSet->bWaitResponse	= TRUE;
			return FALSE;
		}
	}
