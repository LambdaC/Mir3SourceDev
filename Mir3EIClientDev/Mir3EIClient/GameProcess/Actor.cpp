		...
		case _GENDER_MON:
			{	
				...
				else
				{
					...
					m_dwFstFrame = g_xSpriteInfo.m_stMonSpr[bMtn].wFstFrm + (pstFeature->bDress%10)*_MAX_MON_FRAME + bDir*10 -1;
...
BOOL CActor::SetMotionFrame(BYTE bMtn, BYTE bDir)
{
	...
	case _GENDER_MON:
		...
		default:
			...
			m_dwFstFrame = g_xSpriteInfo.m_stMonSpr[bMtn].wFstFrm + (m_stFeature.bDress%10)*_MAX_MON_FRAME + bDir*10 -1;
			m_dwEndFrame = m_dwFstFrame + g_xSpriteInfo.m_stMonSpr[bMtn].wFrmCnt;
			m_wDelay = g_xSpriteInfo.m_stMonSpr[bMtn].wDelay;
		}
		break;
...
VOID CActor::PlayActSound()
{
	...
	if ( nWaveNum != -1 )
		g_xSound.PlayActorWav(m_wPosX, m_wPosY, g_xGameProc.m_pMyHero->m_wPosX, g_xGameProc.m_pMyHero->m_wPosY, nWaveNum);
...
{
	BYTE bDir;
	m_wPosX = lpPacketMsg->stDefMsg.wParam;//ydq,fix
...
VOID CActor::UpdateMotionState(INT nLoopTime, BOOL bIsMoveTime)
{
	...
	if ( m_bCurrMtn == _MT_MON_DIE && m_dwCurrFrame >= m_dwEndFrame-2 )
	{
		m_bIsDead = TRUE;
	}
	...
	if ( m_pxActorImage )
	{
		...
		if ( m_pxActorImage->NewSetIndex(dwCurrFrm) )
		{
			...
			if ( bDrawShadow )
			{
				...
				pxSahdowImage = &g_xGameProc.m_xImage.m_xImageList[m_bActorImgIdx/* ydq +_MAX_MONSTER_IMAGE*/];
...
VOID CActor::DrawHPBar()
{
	...
	if( m_wMAXHP > 0 && !m_bIsDead )
	{
		...
		g_xMainWnd.PutsHan(g_xMainWnd.GetBackBuffer(),
			m_shScrnPosX+7,
			m_shScrnPosY-62,
			RGB(255, 255, 255), RGB(0, 0, 0), szHealth);
	}
}
...
VOID CActor::DrawName()
{
	...
	g_xMainWnd.PutsHan(NULL, rc, RGB(10, 10, 10), RGB(0, 0, 0), m_szName);//綾緞
				   ...
				   FEATURE* pstFeature, FEATUREEX* pstFeatureEx)
{
	if ( CActor::Create(pxImgHandler, pstFeature, bMtn, bDir, wPosX, wPosY) )
	{
		...
		if ( m_stFeature.bHair	  >= _MAX_HAIR )	
			m_stFeature.bHair = (m_stFeature.bHair & 0x0F);

		if ( m_stFeature.bWeapon  >= _MAX_WEAPON )	
...
VOID CHero::PlayActSound()
{
	...
	else if ( m_dwCurrFrame == m_dwFstFrame+1 && m_bCurrMtn >= _MT_ONEVSWING && m_bCurrMtn <= _MT_SPEARHSWING )
	{
		...
		else if ( m_bYedoCnt == 2 )
		{
			if ( m_stFeature.bGender == _GENDER_MAN )
			{
				g_xSound.PlayActorWav(m_wPosX, m_wPosY, g_xGameProc.m_pMyHero->m_wPosX, g_xGameProc.m_pMyHero->m_wPosY, 130);
			}
			else
			{
				g_xSound.PlayActorWav(m_wPosX, m_wPosY, g_xGameProc.m_pMyHero->m_wPosX, g_xGameProc.m_pMyHero->m_wPosY, 131);
			}
			m_bYedoCnt = 0;
		}
...
VOID CHero::PlayMoveSound()
{
	if ( m_bCurrMtn == _MT_WALK || m_bCurrMtn == _MT_RUN || m_bCurrMtn == _MT_HORSEWALK || m_bCurrMtn == _MT_HORSERUN || m_bCurrMtn == _MT_MOODEPO || m_bCurrMtn == _MT_PUSHBACK )
	{
		...
		if ( m_dwCurrFrame == m_dwEndFrame-5 || m_bBackStepFrame == m_dwEndFrame-5 )
			g_xSound.PlayActorWav(m_wPosX, m_wPosY, g_xGameProc.m_pMyHero->m_wPosX, g_xGameProc.m_pMyHero->m_wPosY, 1,100);
...
VOID CHero::OnFeatureChanged(LPPACKETMSG lpPacketMsg)
{
	...
	//TEST
	fnDecode6BitBuf( lpPacketMsg->szEncodeData, (CHAR*)&m_stFeatureEx, sizeof(FEATUREEX) );
...
//	m_wPosY = lpPacketMsg->stDefMsg.wTag;
	...
	memcpy( &m_stFeatureEx, &stFeatureEx, sizeof(FEATUREEX) );
...
VOID CHero::OnTurn(LPPACKETMSG lpPacketMsg)
{
	...
	if (strlen(lpPacketMsg->szEncodeData) > 21 /*_CHARDESCSIZE*/)
	{
		nPos = fnDecode6BitBuf(&lpPacketMsg->szEncodeData[22], (char*)&m_stFeatureEx , sizeof(FEATUREEX));

		nPos = fnDecode6BitBuf(&lpPacketMsg->szEncodeData[/*_CHARDESCSIZE +_FEATURESIZEEX*/29], m_szName, sizeof(m_szName));
		m_szName[nPos] = '\0';
...
//		m_stFeatureEx = stFeatureEx;
		...
		if ( m_stFeature.bHair	  >= _MAX_HAIR )	
	//	FOR DEBUG	m_stFeature.bHair	= _HAIR_NONE;
			m_stFeature.bHair	= (m_stFeature.bHair & 0x0f);
...
//				         m_stFeatureEx.wDressColor, m_stFeatureEx.wHairColor, bFocused, bOpaRate, wState);
		DrawWithEffected(m_rcHair.left, m_rcHair.top, m_pxHairImage->m_lpstNewCurrWilImageInfo->shWidth, m_pxHairImage->m_lpstNewCurrWilImageInfo->shHeight, (WORD*)m_pxHairImage->m_pbCurrImage, 
			m_stFeatureEx.wHairColor, 0XFFFF, bFocused, bOpaRate, wState);
...
VOID CMyHero::AdjustMyPostion()
{
	...
	m_shScrnPosX = (m_wPosX - m_pxMap->m_shStartViewTileX) * _CELL_WIDTH + _VIEW_CELL_X_START;
	m_shScrnPosY = (m_wPosY - m_pxMap->m_shStartViewTileY) * _CELL_HEIGHT+ _VIEW_CELL_Y_START;
...
*/
	...
	else
	{
		switch ( bMtn )
		{
			...
			{
				bMtn = _MT_MON_STAND;
				if ( bDir != m_bCurrDir )			m_pSocketClient->SendActMsg(CM_TRUN, m_wPosX, m_wPosY, bDir);
	//			m_bMotionLock = TRUE;
				m_wOldPosX = m_wPosX;
				m_wOldPosY = m_wPosY;
				m_bOldDir  = m_bCurrDir;
			...
			case _MT_ONEHSWING:
			{
				...
				SetMotionFrame(_MT_MON_ATTACK_A, bDir);
				m_pSocketClient->SendHitMsg(CM_HIT, lpptPos->x, lpptPos->y, bDir, 0);
				break;
			}
		}
	}
...
//	m_shScrnPosY = _CHAR_CENTER_YPOS - 16;
	...
	if( m_shScrnPosX == 0 && m_shScrnPosY == 0 )
	{
		m_shScrnPosX = (m_wPosX - m_pxMap->m_shStartViewTileX) * _CELL_WIDTH + _VIEW_CELL_X_START;
		m_shScrnPosY = (m_wPosY - m_pxMap->m_shStartViewTileY) * _CELL_HEIGHT+ _VIEW_CELL_Y_START;
	}
		...
		if ( bMagicKey )
		{
			...
			if ( pstMagic )
			{
				if ( (10 + pstMagic->stStdMagic.bDefSpell) <= m_stAbility.wMP )
				{
					...
					g_xGameProc.m_xInterface.m_xClientSysMsg.AddSysMsg("침랬꼇璃");
			}
		}
	}
...
VOID CMyHero::DrawName()
{
	...
	RECT rcPos = {m_rcActor.left + (m_rcActor.right-m_rcActor.left-sizeLen.cx)/2, m_rcActor.top + (m_rcActor.bottom-m_rcActor.top-sizeLen.cy)/2-20, 
		m_rcActor.left + (m_rcActor.right-m_rcActor.left+sizeLen.cx)/2, m_rcActor.top + (m_rcActor.bottom-m_rcActor.top+sizeLen.cy)/2-20};	

	g_xMainWnd.PutsHan(NULL, rcPos, m_bActive ? RGB(0,255,0) : m_dwNameClr, RGB(0, 0, 0), szPos);
