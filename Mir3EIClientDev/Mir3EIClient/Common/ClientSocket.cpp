...
BOOL CClientSocket::ConnectToServer(HWND hWnd, CHAR* szID, INT nPort, DWORD dwMessage)
{
...
VOID CClientSocket::SendRunMsg(WORD wIdent, INT nX, INT nY, INT nDir)
{
	DWORD dwTime = timeGetTime();
	fnMakeDefMessage(&DefMsg, wIdent, MAKELONG(nX, nY), LOWORD(dwTime), nDir, HIWORD(dwTime));
	SendPacket(&DefMsg, NULL);
}
...
{
	DWORD dwTime = timeGetTime();
	fnMakeDefMessage(&DefMsg, wIdent, nItemIndex,  LOWORD(dwTime), 0,  HIWORD(dwTime));
...
LRESULT CClientSocket::OnSocketMessage(WPARAM wParam, LPARAM lParam)
{
	...
	{
		MessageBox(NULL, "ÍøÂç´íÎó", NULL, MB_OK);
	...
	switch ( WSAGETSELECTEVENT(lParam) )
	{
		...
		case FD_READ:
		{
			...
			if (g_bProcState == _GAME_PROC)
			{
				...
				m_PacketQ.PushQ((BYTE*)pszPacket);
			}
			else
			{
				ioctlsocket((SOCKET)wParam, FIONREAD, (u_long *)&nRecv);
...
VOID CClientSocket::OnSelectServer(char *pszServerName)
{
	...
	SendPacket(&DefMsg, pszServerName);
}
...
VOID CClientSocket::OnQueryChar(char* pszUserID, INT nCertifyCode)
{
	char szPacket[32];

	fnMakeDefMessage(&DefMsg, CM_QUERYCHR, 0, 0, 0, 0);

	wsprintf(szPacket, "%s/%d", pszUserID, nCertifyCode);
...
VOID CClientSocket::OnSelChar(char* pszUserID, char *pszCharName)
{
	char szPacket[32];

	fnMakeDefMessage(&DefMsg, CM_SELCHR, 0, 0, 0, 0);

	wsprintf( szPacket, "%s/%s", pszUserID, pszCharName );
...
{
	UINT nIndex = atoi( szItemIndex );
	fnMakeDefMessage(&DefMsg, CM_USERBUYITEM, dwNpcId, LOWORD(nIndex), HIWORD(nIndex), 0);
	SendPacket(&DefMsg, szItemName);
}
...
void CClientSocket::SendEncryption(char *pszKey)
{
	fnMakeDefMessage(&DefMsg, 0x47E, 0, 0, 0, 0);
	SendPacket(&DefMsg, pszKey);
}